...
const UserChatModal: React.FC<UserChatModalProps> = ({
    ...
    const checkForNewMessages = async () => {
        ...
        try {
            ...
            if (newMessages.length > 0) {
                ...
                if (lastMessageId) {
                    ...
                    if (filteredNewMessages.length > 0) {
                        console.log('UserChatModal: Найдены новые сообщения:', filteredNewMessages.length);
            ...
            console.error('UserChatModal: Ошибка проверки новых сообщений:', error);
        }
    };
    ...
    const refreshMessages = async () => {
        ...
        console.log('UserChatModal: Ручное обновление сообщений');
        await loadMessages();
    };

    const loadChat = async () => {
        setLoading(true);
        console.log('UserChatModal: Загрузка чата с пользователем', otherUser.id);
        console.log('UserChatModal: Текущий пользователь:', currentUser);
        console.log('UserChatModal: Собеседник:', otherUser);
        
        try {
            // Пытаемся получить существующий чат
            const existingChat = await chatService.getChatByUser(otherUser.id);
            console.log('UserChatModal: Найден существующий чат:', existingChat);
            setChat(existingChat);
        } catch (error: any) {
            console.log('UserChatModal: Ошибка при поиске чата:', error);
            if (error.response?.status === 404) {
                console.log('UserChatModal: Чат не найден, создаем новый');
                // Чат не найден, создаем новый
                await createChat();
            } else {
                console.error('UserChatModal: Неожиданная ошибка загрузки чата:', error);
    ...
    const createChat = async () => {
        console.log('UserChatModal: Создание нового чата с пользователем');
        try {
            const chatData: CreateChatData = {
                participants: [currentUser.id, otherUser.id]
                // Не указываем trade_offer для обычных чатов между пользователями
            };
            
            console.log('UserChatModal: Данные для создания чата:', chatData);
            const newChat = await chatService.createChat(chatData);
            console.log('UserChatModal: Чат успешно создан:', newChat);
            setChat(newChat);
        } catch (error: any) {
            console.error('UserChatModal: Ошибка создания чата:', error);
            console.error('UserChatModal: Подробности ошибки:', error.response?.data);
    ...
    const loadMessages = async () => {
        ...
        try {
            ...
            console.error('UserChatModal: Ошибка загрузки сообщений:', error);
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            console.error('UserChatModal: Ошибка отправки сообщения:', error);
                            ...
                            {messagesLoading ? (
                                ...
                                <View style={styles.emptyState}>
                                    ...
                                    <Text style={styles.emptySubtext}>
                                        Начните беседу с {otherUser.full_name || otherUser.username}
                                    </Text>
                                </View>
                            ) : (
                                <FlatList
